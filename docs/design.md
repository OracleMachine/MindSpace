...
The CLI is invoked with `GEMINI_CLI_HOME=<BASE_STORAGE_PATH>` set in the subprocess env. Gemini CLI resolves `$GEMINI_CLI_HOME/.gemini/`, so the bot's isolated home is `<KB>/.gemini/`:

- `settings.json` — Gemini CLI config (hooks disabled, notifications off, telemetry off, MCP servers rendered from the profile). Regenerated by `mcp_bridge.sync_cli_settings()` on every bot launch.
- `oauth_creds.json`, `google_accounts.json` — symlinks back to `~/.gemini/` so the user's Gemini account auth flows through to the bot subprocess.
- `history/`, `tmp/`, `installation_id`, `projects.json` — Gemini CLI's per-session runtime state.

A human running `gemini` inside the same KB directory shares this `.gemini/` home, so manual consolidation, merge, and REPL-style edits on the knowledge base use the same MCP config and project context the bot uses. The whole `.gemini/` tree is gitignored in each KB (runtime state, not knowledge).
